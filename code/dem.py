    ...
    if dem_type == "A":
        ...
        if len(file_name) == 0:
            ...
            if len(file_name) == 0:
                ...
                if len(file_name) == 0:
                    print('DEM5C of ID: '+coordinates_id+' is unavailable.')
...
def get_elevation(lon, lat, dem_type = "A"):
    ...
    IJ_elevation = next(filter(lambda IJ_elevation: IJ_elevation[0][:3] == I_l[:3] and IJ_elevation[1][:3] == J_l[:3] and IJ_elevation[2] == dem_type, IJ_elevations), None)
    if IJ_elevation is None:
        elevation = read_elevation(I_l, J_l, dem_type)
        IJ_elevations.append((I_l[:3], J_l[:3], dem_type, elevation))
    else:
        elevation = IJ_elevation[3]
    elevation_ll = elevation[I_l[3], J_l[3]]
    
    IJ_elevation = next(filter(lambda IJ_elevation: IJ_elevation[0][:3] == I_l[:3] and IJ_elevation[1][:3] == J_u[:3] and IJ_elevation[2] == dem_type, IJ_elevations), None)
    if IJ_elevation is None:
        elevation = read_elevation(I_l, J_u, dem_type)
        IJ_elevations.append((I_l[:3], J_u[:3], dem_type, elevation))
    else:
        elevation = IJ_elevation[3]
    elevation_lu = elevation[I_l[3], J_u[3]]
    
    IJ_elevation = next(filter(lambda IJ_elevation: IJ_elevation[0][:3] == I_u[:3] and IJ_elevation[1][:3] == J_l[:3] and IJ_elevation[2] == dem_type, IJ_elevations), None)
    if IJ_elevation is None:
        elevation = read_elevation(I_u, J_l, dem_type)
        IJ_elevations.append((I_u[:3], J_l[:3], dem_type, elevation))
    else:
        elevation = IJ_elevation[3]
    elevation_ul = elevation[I_u[3], J_l[3]]
    
    IJ_elevation = next(filter(lambda IJ_elevation: IJ_elevation[0][:3] == I_u[:3] and IJ_elevation[1][:3] == J_u[:3] and IJ_elevation[2] == dem_type, IJ_elevations), None)
    if IJ_elevation is None:
        elevation = read_elevation(I_u, J_u, dem_type)
        IJ_elevations.append((I_u[:3], J_u[:3], dem_type, elevation))
    else:
        elevation = IJ_elevation[3]
