...
"""
与えられた座標の高度を求める関数
read_elevation関数は遅いので，一度読み込んだxmlファイルのデータはelevationsリストに格納しておき，read_elevation関数の使用回数を極力減らしてある
"""
elevations = [] # get_elevation関数が使用するリスト
def get_elevation(lon, lat, dem_type = "A"):
    ...
    elevation = next(filter(lambda elevation: np.array_equal(elevation[0], I_l) and np.array_equal(elevation[1], J_l), elevations), None)
    if elevation is None:
        elevation = read_elevation(I_l, J_l, dem_type)
        elevations.append((I_l, J_l, elevation))
    else:
        elevation = elevation[2]
    elevation_ll = elevation[I_l[3], J_l[3]]
    
    elevation = next(filter(lambda elevation: np.array_equal(elevation[0], I_l) and np.array_equal(elevation[1], J_u), elevations), None)
    if elevation is None:
        elevation = read_elevation(I_l, J_u, dem_type)
        elevations.append((I_l, J_u, elevation))
    else:
        elevation = elevation[2]
    elevation_lu = elevation[I_l[3], J_u[3]]
    
    elevation = next(filter(lambda elevation: np.array_equal(elevation[0], I_u) and np.array_equal(elevation[1], J_l), elevations), None)
    if elevation is None:
        elevation = read_elevation(I_u, J_l, dem_type)
        elevations.append((I_u, J_l, elevation))
    else:
        elevation = elevation[2]
    elevation_ul = elevation[I_u[3], J_l[3]]
    
    elevation = next(filter(lambda elevation: np.array_equal(elevation[0], I_u) and np.array_equal(elevation[1], J_u), elevations), None)
    if elevation is None:
        elevation = read_elevation(I_u, J_u, dem_type)
        elevations.append((I_u, J_u, elevation))
    else:
        elevation = elevation[2]
