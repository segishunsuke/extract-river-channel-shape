...
class RiverCrossSectionExtractor:
    ...
    def read_basic_parameters(self):
        ...
        self.estimate_water_depth = int(data[3][1]) # 水深を推定するか
        self.clear_crossings = int(data[4][1]) # 横断線の交差を解消するか
        self.flow = float(data[5][1]) # 代表地点の平水時の流量[m3/s]
        self.tol1 = float(data[6][1]) # 最低点からtol1メートル以上でなければ堤防の頂点と見なさない
        self.tol2 = float(data[7][1]) # 頂点からtol2メートル下がったら堤防が終わったと見なす
        self.tol3 = float(data[8][1]) # 頂点からtol2メートル下がって，かつ，勾配がtol3以下になったら終了
        self.tol4 = float(data[9][1]) # 頂点が川の端からtol4メートル以上離れている場合はtol1とtol2が高すぎるので調整してやり直し
        self.tol5 = float(data[10][1]) # 頂点と最低点の差がtol5メートル以上の場合はtol1とtol2が高すぎるので調整してやり直し
        self.dem_type = data[11][1] # 使用するdemのtype
        self.distance_between_sections = float(data[12][1]) # 断面取得間隔[m]
        self.transverse_interval = float(data[13][1]) # 横断面のポイント間隔[m]
        self.margin = float(data[14][1]) # 左岸端・右岸端の外側に何メートルのマージンを設けるか
        self.iric_format = int(data[15][1]) # iRICで読み取れる形式で出力するか
        self.adjust1 = float(data[16][1]) # tol1を何倍に調整するか
        self.adjust2 = float(data[17][1]) # tol2を何倍に調整するか
        self.adjust3 = float(data[18][1]) # tol3を何倍に調整するか
        self.water_surface_tolerance = float(data[19][1]) # 水面標高の最小値から何メートルまでの範囲を水面と見なすか[m]
        self.difference_in_differential_equation = float(data[20][1]) # 不等流計算の差分間隔[m]
        self.roughness = float(data[21][1]) # 粗度係数
        self.minimum_slope_water = float(data[22][1]) # 水面勾配の最小値
        self.n_samples_for_median_water_surface = int(data[23][1]) # 水面標高の中央値を計算する際のサンプル数
        self.n_samples_for_median_riverbed = int(data[24][1]) # 水深標高の中央値を計算する際のサンプル数
                ...
                while True: # 右岸側の横断面データが完成するまで繰り返す
                    ...
                    if section_topography_right.size == j:
                        if self.dem_types[i_section][1] == "1A":
                            elevation = dem1a.get_elevation(current[0], current[1]) # 点jの標高を取得
                        else:
                    ...
                    if elevation != -9999.0: # 標高データが正常値の場合（データ欠損は-9999.0）
    ...
    def calculate_water_surface(self):
        ...
        for i_section in range(self.n_sections):
            ...
            if self.wsts[i_section] == 0.0:
                """
                DEMの2022年以前版・2023年以降版の判定（2022年以前は水域の標高は-9999.0，2023年以降は水面標高），この判定は万能ではない（2023年以降も-9999.0を含むことあり）
                """
                min_elevation = np.min(segment)
                if min_elevation == -9999.0:
                    # 2022年以前版
                    self.widths_river[i_section] = np.count_nonzero(segment == -9999.0) * self.transverse_interval # 杭に挟まれた区間内の水面の幅
                    self.elevations_water_tmp[i_section] = np.min(segment[segment != -9999.0]) # 杭に挟まれた区間内の標高の最小値を水面の標高の近似値とする
                else:
                    # 2023年以降版
                    self.widths_river[i_section] = np.count_nonzero(segment <= min_elevation + self.water_surface_tolerance) * self.transverse_interval # 杭に挟まれた区間内の水面の幅
                    self.elevations_water_tmp[i_section] = min_elevation # 杭に挟まれた区間内の標高の最小値を水面の標高の近似値とする
            else:
                """
                wsts > 0.0に設定されている場合は，-9999.0の有無に関わらず必ず2023年以降版として扱う
                """
                self.widths_river[i_section] = np.count_nonzero(segment <= min_elevation + self.wsts[i_section]) * self.transverse_interval # 杭に挟まれた区間内の水面の幅
                self.elevations_water_tmp[i_section] = min_elevation # 杭に挟まれた区間内の標高の最小値を水面の標高の近似値とする
        
        self.elevations_water = np.zeros(self.n_sections)
        
        for i in range(self.n_sections):
            r = min(self.n_samples_for_median_water_surface // 2, i, self.n_sections - 1 - i) # 自身±rの範囲の横断面を考える
    ...
    def calculate_riverbed(self, progress_callback=None):
        ...
        for i in range(self.n_sections):
            ...
            self.elevations_riverbed[i] = np.median(self.elevations_riverbed_tmp[i - r:i + r + 1]) # 自身±rの範囲の横断面を考える
        ...
        for i in range(self.n_sections):
            ...
            if self.wsts[i] == 0.0:
                ...
                if min_elevation == -9999.0:
                    ...
                    ) # 河床底面を設定（elevation_riverbedよりも低い点を河床底面の標高に切り上げ）
                else:
                    # 2023年以降版
