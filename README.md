...
回転角のデフォルト値は0度で，これは，横断面を河道中心線に対して垂直に配置することを意味します．

初回実行時に必要な設定は流量設定のみです．

##### 4-7-3-2. J-FlwDirを利用した流量設定

「J-FlwDirを利用し流量設定」ボタンをクリックすると，[J-FlwDir](https://hydro.iis.u-tokyo.ac.jp/~yamadai/JapanDir/)の上流集水面積のデータを利用して，横断面別の平水流量を設定できます．
...
`iRIC format`が1の場合には，以下のファイルが出力されます．
