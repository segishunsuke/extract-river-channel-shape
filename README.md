...
「基本パラメータ」タブでは，basic_parameters.csvの編集が行えます．設定を変更したいセルをダブルクリックし，値を書き換えてEnterキーを押すことにより編集できます．「保存」ボタンをクリックするとbasic_parameters.csvの上書き保存が行われます．
...
setting.csvを編集したい場合には，設定を変更したいセルをダブルクリックし，値を書き換えてEnterキーを押すことにより編集できます．「横断面別設定を保存」ボタンをクリックするとsetting.csvの上書き保存が行われます．setting.csvの対話的な編集方法については，[5](./README.md#5)で説明します．初回実行時には，setting.csvを編集する必要はありません．
...
22.000kの横断面の右岸側が堤防に届いていません．right_bankの標高を確認すると，6.522mとなっています．これは，隣接する横断面のright_bankの標高（10.785m, 10.741m）と比べても低いです．
...
上記の例では，右岸側のtol1が低過ぎることが原因だと考えられたため，22.000kのRight tol1を5m→7.5mに変更しました．

<img src="./assets/images/edit1.png">

22.000kのUse intermediate resultを0に変更し，「横断面別設定を保存」ボタンをクリックしてから，河道横断面抽出を再度実行した結果，横断面が以下のように修正されました．
...
### 5-2. 横断面が堤内地の奥まで伸びている

以下の図の例を見て下さい．

<img src="./assets/images/case2.png">

8.400kとその周辺の横断面の右岸側が，（Marginの設定値を超えて）堤防を超過しています．
...
上記の例では，右岸側のtol2が高すぎる，もしくは，tol3が低過ぎることが原因だと考えられたため，8.400kとその周辺の横断面のRight tol2を2m→0.1mに，Right tol3を0.1→1に変更しました．
...
このような場所では，河道の範囲が曖昧であるため，左岸端・右岸端の位置の設定は裁量的に行えるケースが多いと考えられます．
...
そのため，「DEMに標高が記録されていない陸上の場所」が横断面上に存在すると，横断面形の設定が適切に行えません．
