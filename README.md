...
すると，以下のようなウィンドウが立ち上がります．このGUI上でプログラムの操作を行います．
...
「横断面別設定・実行」タブの上部には，`settings.csv`のうち，重要性の高い項目が表示されています．

各行が一つの横断面に対応しています．各列のパラメータの定義は以下の通りです．

- 距離: 下流端からの距離(km)
- 前回結果使用: 前回のプログラム実行時に読み取った標高データを用いる（1は使用，0は使用せず読み取り直し）
- 流量: 平水流量(m3/s)
- 回転角: 横断線の調整角度(反時計回り，度)
- 左手動，右手動: 横断線の範囲設定を自動化せず手動で与える
- 左tol2，右tol2: 左岸端，右岸端のtol2の設定（0は堤防無しを意味する）
