...
横断線が支流の奥に向かって伸びているなど，その向きが不適切であるケースがあります．

このようなケースにおいては，以下の対応を行って下さい．

- Angle adjustmentを適宜設定し，Use intermediate resultを0に設定する

デフォルトのAngle adjustment = 0では，横断線が河道中心線に直交します．

Angle adjustmentを変更すると，河道中心線上の点を軸として，横断線が回転します．

Angle adjustmentの単位は度（ラジアンではない）で，反時計回りの方向を正とします．よって，Angle adjustmentを増やせば，右岸端は下流側へ，左岸端は上流側へ移動します．

### 5-4. setting.csvの事前作成

以上では，プログラムの初回実行時に出力されるsetting.csvを編集することを前提に説明を行いました．

プログラムの初回実行時よりも前に，setting.csvを手作業で作成しておくこともできます．

setting.csvを事前に作成するメリットはさほどありませんが，回転しなければ横断面の設定ができない横断線がある場合（例えば，河口部で回転しないと左岸に陸地が無い場合など）には役に立ちます．
