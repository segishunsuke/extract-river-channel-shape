...
Flowの設定値は，後の分析で横断面ごとにカスタマイズできます．
...
横断面の河床標高を設定する処理の実行中に，下記のようなエラーメッセージが表示されてプログラムが停止することがあります．

```
距離標Xkm (左岸端座標: X, X 右岸端座標: X, X)において水面が見つかりませんでした
この断面のtol1とtol3を大きく設定して下さい
```

このエラーは，ある横断面について，設定された横断線の範囲内に水面（低水路）が存在しないときに表示されます．

国土数値情報の河道中心線は高水敷の上に引かれていることがあり，このときに上記のような事態が生じることがあります．

このエラーが表示されたときには，以下のように対処して下さい．

1. エラーメッセージに表示されている左岸端座標と右岸端座標を地図上で検索し，左岸側と右岸側のどちらに横断線を延長すれば，横断線の範囲内に水面を含めることができるのかを調べて下さい．
2. "setting.csv"を開き，"Distance"列の数値がエラーメッセージに表示されている距離標に等しい行を探して下さい．"setting.csv"は，上記のエラーメッセージが表示される段階においては出力されています．
3. 横断線を左岸側に延長する場合には，その行の"Left tol1"と"Left tol3"の数値を増やして下さい．"Left tol1"に20，"Left tol3"に1などを

"setting.csv"を開き，エラーメッセージに表示されている距離標の数値を"Distance"列から探して下さい．
